...
with tf.name_scope('summaries'):
    ...
    # concat_images = tf.concat([boxed_labels, boxed_predictions, output_masks], axis=2)
    tf.summary.image('outputs', boxed_labels, max_outputs=5)
...
with tf.Session() as sess:
    ...
    i_v = 0
    while True:
        t = sess.run(merged, feed_dict={handle: training_handle})
        v = sess.run(merged, feed_dict={handle: validation_handle})
        print(i_t)
        train_writer.add_summary(t, i_t)
        train_writer.flush()
        val_writer.add_summary(v, i_t)
        val_writer.flush()
        i_t +=1
